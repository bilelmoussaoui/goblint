...
impl GParamSpecStaticNameCanonical {
    fn check_node(
            ...
            && let Some(function) = node.child_by_field_name("function")
        {
            ...
            {
                let name_text = ast_context.get_node_text(name_arg, ctx.source);

                // Check if the name (which should be a string literal) contains underscores
                // Strip quotes to get the actual name
                ...
                if name_value.contains('_') {
                    ...
                    let fix = Fix::from_node(name_arg, ctx, replacement);
                    ...
                    violations.push(self.violation_with_fix(
                        ctx.file_path,
                        ctx.base_line + name_arg.start_position().row,
                        name_arg.start_position().column + 1,
                        message,
                        fix,
                    ));
                }
